...
function parseEventStart(startStr) {
  const allDayMatch = /^(\d{4})-(\d{2})-(\d{2})$/.exec(startStr);
  if (allDayMatch) {
    return new Date(Number(allDayMatch[1]), Number(allDayMatch[2]) - 1, Number(allDayMatch[3]));
  }
...
function pickNextOccurrence(events, calendarName, uid, nowMs) {
  ...
  const matches = events
    ...
    .filter((e) => parseEventStart(e.start).getTime() >= now)
...
function computeCountdown(startIso, nowMs) {
  const now = nowMs != null ? nowMs : Date.now();
  const ms = Math.max(0, parseEventStart(startIso).getTime() - now);
  return {
...
async function findNextOccurrence(config, calendarName, uid) {
  ...
  let events;
  try {
    events = await cache.get(`wide:${calendarName}`, ttl, () => fetchCalendar(cal, now, end));
  } catch (err) {
    console.warn(`findNextOccurrence "${calendarName}" failed: ${err.message}`);
    return null;
  }
